...
def addproduction_2_db():
  ...
  try: 
    for producers in producer_ids:
      ...
      select_cmd = "SELECT prod_id, producer_id FROM Produced_By WHERE prod_id=:p AND producer_id=:r"
...
@app.route('call_sheet')
def call_sheet():
# Take input values
     prod_id=int(request.args['production'])
     scenes = request.form.getlist('scenes')
     scene_ids = map(lambda c: int(c), scenes)

# Data From The Production

     prod_query=("SELECT P.prod_title "
                "FROM Productions as P "
                "WHERE P.prod_id = :p")
     ...
     cursor.close()
     prod_title = production['prod_title']
#The rest of this should be looped for each scene_id  but I didn't really know how to do that, and handle the ouput, I think the queries should be written to output with each pass but I don't know the html
    #   for scene_id in scene_ids:
	
#Data from the scene
     scene_query=("SELECT S.scene_id, S.description"
                      "FROM Scenes as S "
                      "WHERE S.scene_id=:s")

     cursor = g.conn.execute(text(scene_query),s=scene_id)
     scene=cursor.fetchone()
     cursor.close()
     description = scene['description']
#Data from Shot_At
     shot_at_query=("SELECT S.filming_loc, S.shoot_time"
                      "FROM Shot_At as S "
                      "WHERE S.scene_id=:s")

     cursor = g.conn.execute(text(shot_at_query),s=scene_id)
     scene=cursor.fetchone()
     cursor.close()
#Data from Portrays
    select_portrays_query=("SELECT U.name "
...
#Data from Made_Of
    made_of_query=("SELECT M.page_num "
                          "FROM Made_Of as M "
                          "WHERE M.scene_id=:s AND M.prod_id=:p")
    cursor=g.conn.execute(text(made_of_query),s=scene_id, p=prod_id)
    m=cursor.fetchone()
    cursor.close()
    page_range=[int((m['page_num']).lower),int((m['page_num']).upper)]
...
#Data From Works_On
	...
	works_on_query=("SELECT W.cid"
			"FROM Works_On W" 
			"WHERE W.scene_id=:s")
    cursor=g.conn.execute(text(works_on_query),s=scene_id, p=prod_id)
    m=cursor.fetchone()
    cursor.close()
    page_range=[int((m['page_num']).lower),int((m['page_num']).upper)]
